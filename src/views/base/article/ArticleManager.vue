<template>
  <div>
    ...
    <JsonExcel :data="tableData" :fields="fields" worksheet="sheet1" name="文章" :before-generate="beforeDownL"
    ...
    <Paging @curr-page="currPage" :total="total" :pageSize="size"></Paging>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    //当前页发生改变时触发，重新请求数据
    currPage(page) {
      this.getArticle(page, this.size)
    },
    /*
    * 导出excel前函数
    * */
    ...
    * */
    ...
    getArticle(page, size) {
      this.$requestGET({
        url: '/getArticle?page=' + page + "&pageSize=" + size
      }).then(res => {
        if (res.code === 200) {
